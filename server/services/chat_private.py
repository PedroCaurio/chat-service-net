...
def send_private_message(sender_username: str, receiver_username: str, text: str) -> bool:
    ...
    if not UserRepository.get_user_by_username(sender_username):
        return False
    if not UserRepository.get_user_by_username(receiver_username):
        return False

    msg = Message(from_=sender_username, to=receiver_username, text=text)
    conv = conversation_id(sender_username, receiver_username)  # gera o id da conversa
